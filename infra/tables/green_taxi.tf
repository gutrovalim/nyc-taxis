resource "aws_glue_catalog_table" "tb_green_taxi_silver" {
	name          = "tb_green_taxi"
	...
	storage_descriptor {
		location      = "s3://nyc-taxis-silver/green-taxi/"
		input_format  = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
		output_format = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat"

		ser_de_info {
			name                  = "tb_green_taxi_silver"
		...
		columns {
			name    = "VendorID"
			type    = "int"
			comment = "Código do provedor LPEP. 1=Creative Mobile Technologies, LLC; 2=Curb Mobility, LLC; 6=Myle Technologies Inc"
		}
		columns {
		...
		columns {
			...
			type    = "string"
			comment = "Flag de armazenamento e envio"
		}
		columns {
			name    = "RatecodeID"
			type    = "int"
		...
		columns {
			name    = "trip_type"
			type    = "int"
			comment = "Tipo de corrida. 1=Street-hail; 2=Dispatch"
		}
		columns {
    ...
    partition_keys {
        name    = "year_month"
        type    = "int"
        comment = "Ano e mês da corrida no formato YYYYMM"
	}
}
resource "aws_glue_catalog_table" "tb_green_taxi" {
	name          = "tb_green_taxi"
	...
	storage_descriptor {
	location      = "s3://nyc-taxis-bronze/green-taxi/"
		input_format  = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
		output_format = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat"
